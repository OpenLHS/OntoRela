...
COMMENT ON SCHEMA "PDRO_en" IS 'Create views in en of PDRO 20240430-1659';
...
COMMENT ON VIEW "PDRO_en"."treatment" IS 'A processual entity whose completion is hypothesized (by a healthcare provider) to alleviate the signs and symptoms associated with a disorder';
...
COMMENT ON VIEW "PDRO_en"."continuant fiat boundary" IS 'b is a continuant fiat boundary = Def. b is an immaterial entity that is of zero, one or two dimensions and does not include a spatial region as part. (axiom label in BFO2 Reference: [029-001])';
...
CREATE VIEW "PDRO_en"."spatiotemporal region" AS
  SELECT "BFO_0000011_uid" AS "uid spatiotemporal region"
  FROM "PDRO"."BFO_0000011";

COMMENT ON VIEW "PDRO_en"."spatiotemporal region" IS 'SpatiotemporalRegion';

CREATE VIEW "PDRO_en"."process-profile" AS
  SELECT "BFO_0000144_uid" AS "uid process-profile"
  FROM "PDRO"."BFO_0000144";

COMMENT ON VIEW "PDRO_en"."process-profile" IS 'b is a process_profile =Def. there is some process c such that b process_profile_of c (axiom label in BFO2 Reference: [093-002])';
...
CREATE VIEW "PDRO_en"."2d-cf-boundary" AS
  SELECT "BFO_0000146_uid" AS "uid 2d-cf-boundary"
  FROM "PDRO"."BFO_0000146";

COMMENT ON VIEW "PDRO_en"."2d-cf-boundary" IS 'TwoDimensionalContinuantFiatBoundary';
...
COMMENT ON VIEW "PDRO_en"."OGMS_0000090_BFO_0000055_ONTORELA_C3eb44fa7" IS 'A processual entity whose completion is hypothesized (by a healthcare provider) to alleviate the signs and symptoms associated with a disorder null treatmentRealizableEntity A directive information entity with action specifications and objective specifications as parts that, when concretized, is realized in a process in which the bearer tries to achieve the objectives by taking the actions specified. A health care objective specification that specifies a treatment.';
